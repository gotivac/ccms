...
class Menu extends MultilingualActiveRecord
{
	...
	public function rules()
	{
		...
		$rules =  array(
			array('parent_id, created_user_id, updated_user_id', 'numerical', 'integerOnly'=>true),
			array('title, link', 'length', 'max'=>255),
			array('created_dt, updated_dt', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, parent_id, title, link, created_user_id, created_dt, updated_user_id, updated_dt', 'safe', 'on'=>'search'),
		);

        foreach ($this->languages() as $l) {
            $rules[] = array('title_'.$l, 'length', 'max'=>256);
            $rules[] = array('link_'.$l, 'length', 'max'=>256);
        }
	...
	public function relations()
	{
		...
		return array(
			'menuLangs' => array(self::HAS_MANY, 'MenuLang', 'menu_id'),
		);
	}
