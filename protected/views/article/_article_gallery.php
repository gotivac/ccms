...
<h4><?= Yii::t('app','Article Gallery');?></h4>
...
<ul class="visualizacao sortable dropzone-previews">
    <?php
    $article_gallery = ArticleGallery::model()->findAll(array('condition'=>'article_id='.$model->id,'order'=>'priority ASC'));
    ?>

    <?php foreach ($article_gallery as $image): ?>
        <li>
            <div>
                <div class="dz-preview dz-file-preview">
                    <?= CHtml::image(Yii::app()->baseUrl.$image->filepath, $image->filename,array('class' => 'img-responsive')); ?>
                </div>
            </div>
...
<script>
    $(document).ready(function(){
        ...
        $('.dz-remove').on('click',function(){
            ...
            $.ajax({
                url: '<?=Yii::app()->createUrl('article/removeImage');?>' + '/' + id,
                type: 'post',
